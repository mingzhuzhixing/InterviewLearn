...
public class RichEditText extends MentionEditText {
     ...
     * @param userModel 用户实体
     */
    public void newResolveText(UserModel userModel) {
        String userName = userModel.getUser_name();
        userModel.setUser_name(userName + "\b");
        int index = getSelectionStart();
        Editable editable = getText();
        editable.insert(index, userName);
        editable.insert(index + userName.length(), "\b");

        SpannableString spannableString = new SpannableString(getText());
        MyForegroundSpan mySpan = new MyForegroundSpan(Color.parseColor(colorAtUser), userModel.getUser_id(), userName);
        spannableString.setSpan(mySpan, index, index + userName.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
     ...
     * @param userModel       用户model
     */
    public void newResolveAtResult(UserModel userModel) {
        String user_id = userModel.getUser_id();
        String user_name = "@" + userModel.getUser_name();
        UserModel user = new UserModel(user_name, user_id);
        newResolveText(user);
     ...
     * 删除制定的@用户
     */
    public void deleteAtResult(UserModel userModel) {
        if (userModel == null) {
            return;
        }
        try {
            String showText = "@" + userModel.getUser_name();
            String showId = userModel.getUser_id();
            Editable editable = getText();
            Spannable spannableString = new SpannableString(editable);
            String content = editable.toString();
            for (int i = -1; i <= content.lastIndexOf(showText); ++i) {
                i = content.indexOf(showText, i);
                MyForegroundSpan[] spans = spannableString.getSpans(i, i + showText.length(), MyForegroundSpan.class);
                if (spans != null && spans.length > 0) {
                    if (showId.equals(spans[0].getShowId())) {
                        SpannableStringBuilder ssb = new SpannableStringBuilder(getText());
                        ssb.replace(i, i + showText.length() + 1, "");//加1去掉结尾空格
                        setText(ssb);
     ...
     * 获取所有的@的用户
     */
    public List<UserModel> getAllAtUser() {
        List<UserModel> userModelList = new ArrayList<>();
        try {
            Spannable spannableString = new SpannableString(getText());
            MyForegroundSpan[] spans = spannableString.getSpans(0, spannableString.length(), MyForegroundSpan.class);
            if (spans != null && spans.length > 0) {
                for (MyForegroundSpan foregroundSpan : spans) {
                    if (foregroundSpan != null) {
                        userModelList.add(new UserModel(foregroundSpan.showText, foregroundSpan.showId));
                    }
                }
            }
        } catch (Exception e) {
     ...
     * 提交真实文本可以替换了\b
     */
    public String getNewRealText() {
        try {
            Spannable spannableString = new SpannableString(getText());
            StringBuilder builder = new StringBuilder(spannableString.toString());
            //Log.i(TAG, "content:" + builder.toString());
            MyForegroundSpan[] spans = spannableString.getSpans(0, spannableString.length(), MyForegroundSpan.class);
            if (spans != null && spans.length > 0) {
                for (MyForegroundSpan foregroundSpan : spans) {
                    if (foregroundSpan != null) {
                        String oldStr = foregroundSpan.showText;
                        String newStr = "<at uid=\"" + foregroundSpan.getShowId() + "\">" + foregroundSpan.getUserName() + "</at> ";
                        //Log.i(TAG, "oldStr:" + oldStr + "=newStr:" + newStr + "==");
                        int start = builder.indexOf(oldStr);
                        int end = start + oldStr.length();
                        if (start >= 0 && start <= end) {
                            builder.replace(start, end, newStr);
                        }
                        //Log.i(TAG, "replace_content:" + builder.toString());
                    }
                }
            }
    ...
    public static class MyForegroundSpan extends ForegroundColorSpan implements Serializable {
        private String showId;
        private String showText;
        private String userName;

        public MyForegroundSpan(int color) {
            super(color);
        }

        public MyForegroundSpan(int color, String showId, String showText) {
            super(color);
            this.showId = showId;
            this.showText = showText;
            if (!TextUtils.isEmpty(showText) && showText.startsWith("@")) {
                this.userName = showText.substring(1);
            }
        }
