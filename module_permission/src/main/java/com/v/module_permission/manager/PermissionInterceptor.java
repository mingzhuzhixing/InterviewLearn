package com.v.module_permission.manager;
...
import com.v.module_permission.R;
...
public final class PermissionInterceptor implements IPermissionInterceptor {
    ...
    public void deniedPermissionRequest(@NonNull Activity activity, @NonNull List<String> allPermissions,
        ...
        if (doNotAskAgain) {
            if (deniedPermissions.size() == 1 && Permission.ACCESS_MEDIA_LOCATION.equals(deniedPermissions.get(0))) {
                ToastUtils.showToast(R.string.common_permission_media_location_hint_fail);
                return;
            }
        ...
        if (deniedPermissions.size() == 1) {
            ...
            if (Permission.ACCESS_BACKGROUND_LOCATION.equals(deniedPermission)) {
                ToastUtils.showToast(activity.getString(R.string.common_permission_background_location_fail_hint, backgroundPermissionOptionLabel));
                return;
            }

            if (Permission.BODY_SENSORS_BACKGROUND.equals(deniedPermission)) {
                ToastUtils.showToast(activity.getString(R.string.common_permission_background_sensors_fail_hint, backgroundPermissionOptionLabel));
                return;
            }
        }
    ...
    private void showPopupWindow(Activity activity, ViewGroup decorView, String message) {
        if (mPermissionPopup == null) {
            View contentView = LayoutInflater.from(activity).inflate(R.layout.permission_description_popup, decorView, false);
